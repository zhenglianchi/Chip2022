...
Different from the commonly used GAN-based data synthesis tools_, the main framework of Style-Text includes:
...
1. You can run `tools_/synth_image` and generate the demo image, which is saved in the current folder.

```python
python3 tools_/synth_image.py -c configs/config.yml --style_image examples/style_images/2.jpg --text_corpus PaddleOCR --language en
```
...
2. You can run the following command to start synthesis task:
   ...
   ``` bash
   python3 tools_/synth_dataset.py -c configs/dataset_config.yml
   ```
