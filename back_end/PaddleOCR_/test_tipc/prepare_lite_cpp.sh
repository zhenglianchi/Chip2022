...
if [[ ${paddlelite_library_source} = "download" ]]; then
    ...
    cd Paddle-Lite
    ./lite/tools_/build_android.sh  --arch=armv8  --with_cv=ON --with_extra=ON --toolchain=clang --with_opencl=${compile_with_opencl}
    cd ../
...
cp ppocr_/utils/ppocr_keys_v1.txt deploy/lite/config.txt ${paddlelite_library_file}/demo/cxx/ocr/test_lite
