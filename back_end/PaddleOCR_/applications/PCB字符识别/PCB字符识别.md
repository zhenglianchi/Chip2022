...
```python
cd /home/aistudio/PaddleOCR/
python3 tools_/train.py -c configs/rec/PP-OCRv3/ch_PP-OCRv3_rec.yml
```
...
```python
# 导出检测模型
python3 tools_/export_model.py \
     -c configs/det/ch_PP-OCRv3/ch_PP-OCRv3_det_student.yml \
...
# 检测模型inference模型预测
cd /home/aistudio/PaddleOCR/
python3 tools_/infer/predict_det.py \
...
# 识别模型inference模型预测
cd /home/aistudio/PaddleOCR/
python3 tools_/infer/predict_rec.py \
...
cd /home/aistudio/PaddleOCR/
python3 tools_/infer/predict_system.py  \
...
1）首先运行`tools_/infer/predict_system.py`，将`image_dir`改为需要评估的数据文件家，得到保存的结果:
...
2）然后将步骤一保存的数据转换为端对端评测需要的数据格式： 修改 `tools_/end2end/convert_ppocr_label.py`中的代码，convert_label函数中设置输入标签路径，Mode，保存标签路径等，对预测数据的GTlabel和预测结果的label格式进行转换。
```
...
convert_label(ppocr_label_gt, "pred", "./save_ppocr_V2_infer/")
```
...
```python
 python3 tools_/end2end/convert_ppocr_label.py
```
...
3） 最后，执行端对端评测，运行`tools_/end2end/eval_end2end.py`计算端对端指标，运行方式如下：


```python
pip install editdistance
python3 tools_/end2end/eval_end2end.py ./save_gt_label/ ./save_ppocr_V2_infer/
```
