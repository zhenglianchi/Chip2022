...
python3 -m paddle.distributed.launch --gpus '0,1,2,3' tools_/train.py -c configs/det/det_mv3_db.yml \
     -o Global.pretrained_model=./pretrain_models/MobileNetV3_large_x0_5_pretrained
...
```shell
python3 tools_/train.py -c configs/det/det_mv3_db.yml -o Optimizer.base_lr=0.0001
```
...
PaddleOCR将网络划分为四部分，分别在[ppocr_/modeling](../../ppocr_/modeling)下。 进入网络的数据将按照顺序(transforms->backbones->
...
```shell
python3 tools_/eval.py -c configs/det/det_mv3_db.yml  -o Global.checkpoints="{path/to/weights}/best_accuracy"
```
