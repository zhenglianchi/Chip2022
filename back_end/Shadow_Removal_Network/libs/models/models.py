...
def benet(pretrained: bool = False, **kwargs: Any) -> BENet:
    model = BENet(**kwargs)
    if pretrained:
        state_dict = torch.load("Shadow_Removal_Network/pretrained/pretrained_benet.prm",map_location=torch.device('cpu'))  # map_location
...
def generator(pretrained: bool = False, **kwargs: Any) -> Generator:
    ...
    if pretrained:
        state_dict = torch.load("Shadow_Removal_Network/pretrained/pretrained_g_srnet.prm",map_location=torch.device('cpu'))
        model.load_state_dict(fix_model_state_dict(state_dict))
    return model
...
def discriminator(pretrained: bool = False, **kwargs: Any) -> Discriminator:
    ...
    if pretrained:
        state_dict = torch.load("Shadow_Removal_Network/pretrained/pretrained_d_srnet.prm",map_location=torch.device('cpu'))
        model.load_state_dict(fix_model_state_dict(state_dict))
    return model
